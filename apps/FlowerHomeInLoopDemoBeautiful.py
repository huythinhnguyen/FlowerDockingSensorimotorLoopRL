...
def set_up_waveform_plot(ax: plt.Axes, distances: ArrayLike, data_left: ArrayLike, data_right: ArrayLike, title: Optional[str]=None, fontsize: int=8):
    line_left, = ax.plot(distances, data_left, linewidth=1., alpha=0.5, label=TRANSLATED_LABELS['left'])
    line_right, = ax.plot(distances, data_right, linewidth=1., alpha=0.5, label=TRANSLATED_LABELS['right'])
...
def mainapp():
    ...
    flower_est_asset = mpimg.imread(FLOWER_EST_ASSET_PATH)
    bat_asset = mpimg.imread(BAT_ASSET_PATH)
    bat_hit_asset = mpimg.imread(BAT_HIT_ASSET_PATH)
    for i in range(len(cartesian_objects_matrix)):
        rotated_flower_asset = ndimage.rotate(flower_asset, np.degrees(cartesian_objects_matrix[i,2]))
        flower_imgbox = OffsetImage(rotated_flower_asset, zoom=ASSET_SCALE)
        flower_annobox = AnnotationBbox(flower_imgbox, (cartesian_objects_matrix[i,0], cartesian_objects_matrix[i,1]), frameon = False)
    ...
    flower_est_artists.append(ax1.add_artist(flower_est_annobox))    
    ...
    ax2.plot(compressed_distances, pose_estimators[estimator_type].presence_detector.profile*5, 'k--', linewidth=.5, alpha=0.5)
    ...
    flowers_checkbox_ax = fig.add_axes([ 0.75, 0.6, 0.1, 0.35])
    estimator_type_radio_ax = fig.add_axes([ 0.85, 0.7, 0.1, 0.2])
    flowers_slider_ax = []
    ...
    reset_button_ax = fig.add_axes([0.55, 0.85, 0.1, 0.05])  #([0.92, 0.85, 0.05, 0.05])
    run_episode_button_ax = fig.add_axes([0.55, 0.9, 0.1, 0.05])
    ...
    eps_progress_slider = widgets.Slider(eps_progress_slider_ax, 'Ep. progress', 0, 10, valstep=1, valinit=0)
    ...
    def update(val):
        ...
        ##### Edit image here ################
        for i in range(len(cartesian_objects_matrix)):
            flower_artists[i].remove()
            rotated_flower_asset = ndimage.rotate(flower_asset, np.degrees(cartesian_objects_matrix[i,2]))
            flower_imgbox = OffsetImage(rotated_flower_asset, zoom=ASSET_SCALE)
            flower_annobox = AnnotationBbox(flower_imgbox, (cartesian_objects_matrix[i,0], cartesian_objects_matrix[i,1]), frameon = False)
            flower_artists[i] = ax1.add_artist(flower_annobox)
            flower_artists[i].set_visible(flower_status[i])
            for w in range(3): flowers_slider_ax[i*3 + w].set_visible(flower_status[i])
