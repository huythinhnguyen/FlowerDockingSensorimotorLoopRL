...
from TrajectoryHandler.dubinspath import DubinsToKinematics, DubinsDockZonePathPlanner
...
def mainapp(init_v:float):
    ...
    def run_path(event):
        ...
        dockzone_circle.set_data(*generate_dockzone_notched_circle_waypoints(dockzone))
        v_seq, w_seq = kinematic_converter(path=path, init_v = init_v)
...
def main():
    #return mainapp()
    if len(sys.argv)==1: return mainapp(init_v=0.)
    return mainapp(init_v=float(sys.argv[1]))
