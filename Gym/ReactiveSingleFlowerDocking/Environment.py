...
class DiscreteActionSimpleFlowerDocking(py_environment.PyEnvironment):
    def __init__(self):
        init_pose = np.asarray([0, -2, np.radians(90)])
        self.objects = np.asarray([[0, 0, np.radians(90)]])
        self.motion = State(pose = init_pose, dt=1/CHIRP_RATE)
        self.render = Spatializer.Render()
        self.controller = DiscreteActions1()
...
class DiscreteAction(py_environment.PyEnvironment):
    def __init__(self, time_limit, level=0, phase=0, difficulty=0, mode='box', max_level=1, log=False):
        ...
        self.locomotion = State(pose = init_pose, dt=1/CHIRP_RATE)
