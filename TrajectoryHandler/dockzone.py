...
def generate_dockzone_notched_circle_waypoints(dockzone_circle: DockZoneNotchedCircle, num_waypoints: int = 100, **kwargs) -> Tuple[ArrayLike, ArrayLike]:
    ...
    left_center  = center + radius * np.array([np.cos(dockzone_circle.theta + np.pi/2), np.sin(dockzone_circle.theta + np.pi/2)])
    right_center = center + radius * np.array([np.cos(dockzone_circle.theta - np.pi/2), np.sin(dockzone_circle.theta - np.pi/2)])
    back_thetas = np.linspace(-np.pi*0.5, np.pi*0.5, int(num_waypoints/4)) + dockzone_circle.theta
    front_thetas = np.linspace(-np.pi*1.5, -np.pi*0.5, int(num_waypoints/2)) + dockzone_circle.theta
    x = np.concatenate([
