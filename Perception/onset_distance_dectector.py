...
class ClosestPeakDistance(PerceptionBase):
    ...
    def run(self, x, ouput_index=False):
        ...
        left_peaks, _ = find_peaks(inputs[0,0,:], distance= int(TransformConfig.ENVELOPE_LENGTH/16), prominence=0.01, height=self.profile)
        right_peaks, _ = find_peaks(inputs[0,1,:], distance= int(TransformConfig.ENVELOPE_LENGTH/16), prominence=0.01, height=self.profile)
        candidates = []
        if len(left_peaks) == 0 and len(right_peaks) == 0: return None
        if len(left_peaks) > 0 : candidates.append(left_peaks[np.argmax(inputs[0,0,left_peaks])])
        if len(right_peaks) > 0 : candidates.append(right_peaks[np.argmax(inputs[0,1,right_peaks ])])
