...
class DiscreteActions1:
    ...
    def __init__(self, v=0.5, w=np.pi/2):
        self._v = v
        self._w = w
        self._cmd_dict = {0: (0, w), 1: (v, 0), 2: (0, -w)}

    def __call__(self, action):
        return self.run(action)
    
    def run(self, action):
        if type(action)==np.ndarray: action = int(action)
...
class DiscreteActions2:
    def __init__(self, max_v=1, max_w = np.pi):
        self._max_v = max_v
        self._max_w = max_w
        self._cmd_dict = {0: (0, max_w), 1: (0.5*max_v, 0.5*max_w),
                          2: (max_v, 0), 3: -(0.5*max_v, -0.5*max_w), 4:(0, max_w)}
