...
class DubinsDockZonePathPlanner:
    ...
    def add_waypoints_to_path(self, path:DubinsParams, n_points:int=50) -> DubinsParams:
        ...
        for i in range(1, len(path.modes)):
            if path.modes[i] == 'S':
                x = np.concatenate((x, np.linspace(path.tangent_points[i-1][0], path.tangent_points[i][0], n_points)))
                y = np.concatenate((y, np.linspace(path.tangent_points[i-1][1], path.tangent_points[i][1], n_points)))
            else:
                alpha = np.arctan2(path.tangent_points[i-1][1] - path.centers[i][1], path.tangent_points[i-1][0] - path.centers[i][0])
    ...
    def _find_tangent_points(self, centers: List[ArrayLike], modes: List[str], radii: List[float], dockzone_theta:float) -> Tuple[List[ArrayLike]]:
        ...
        # TODO: THIS IS SO POORLY WRITTEN. MAY NEED TO REWRITE THIS PART ELEGANTLY WHEN I HAVE TIME.
        if np.sum(valids) == 0:
            if modes[0]!=modes[-1]: return [None]*(len(centers)-1), centers, radii, modes
            candidate_tangent_points, candidate_centers, candidate_radii, candidates_modes = [], [], [], []
            temp0, temp1, temp2, temp3 = self._find_tangent_points_one_inside_another(centers[:2]+[centers[2]], modes[:2]+[modes[2]], radii[:2]+[radii[2]])
            candidate_tangent_points.append(temp0)
            candidate_centers.append(temp1)
            candidate_radii.append(temp2)
            candidates_modes.append(temp3)
            temp0, temp1, temp2, temp3 = self._find_tangent_points_one_inside_another(centers[:2]+[centers[3]], modes[:2]+[modes[3]], radii[:2]+[radii[3]])
            candidate_tangent_points.append(temp0)
            candidate_centers.append(temp1)
            candidate_radii.append(temp2)
            candidates_modes.append(temp3)
            valids = []
            valids.append(np.any(candidate_tangent_points[0][-1])!=None)
            valids.append(np.any(candidate_tangent_points[1][-1])!=None)
            if np.sum(valids) == 0: raise ValueError('No valid tangent points found. In the one inside another case. I thought I failed proofed everything')
            if np.sum(valids) == 2: print('Warning: There are 2 valid tangent points. I did not anticipate this case.')
            if valids[0]:
                return candidate_tangent_points[0], candidate_centers[0], candidate_radii[0], candidates_modes[0]
            else:
                new_centers = candidate_centers[1] + [centers[2]]
                new_radii = candidate_radii[1] + [radii[2]]
                ...
                new_tangent_points = candidate_tangent_points[1] + [intersection_between_two_circles]
                new_modes = candidates_modes[1] + [modes[2]]
                return new_tangent_points, new_centers, new_radii, new_modes
    ...
    def _find_tangent_points_one_inside_another(self, centers: List[ArrayLike], modes: List[str], radii: List[float]) -> List[ArrayLike]:
        d = np.linalg.norm(centers[0] - centers[2])
        # find the intermediate circle, center and radius
        if radii[0] < radii[2]: order_idx = [0, 2]
        else: order_idx = [2, 0]
        if d > radii[order_idx[1]] - radii[order_idx[0]]: 
            return [None]*len(centers), centers, radii, modes
        intermediate_radius = radii[order_idx[0]]
        temp_theta = np.arctan2(centers[order_idx[1]][1] - centers[order_idx[0]][1], centers[order_idx[1]][0] - centers[order_idx[0]][0])
        intermediate_center = centers[order_idx[1]] + (radii[order_idx[1]] - radii[order_idx[1]])*np.array([np.cos(temp_theta), np.sin(temp_theta)])
        # find the tangent points between the small circle and intermediate circle
        new_centers = centers[:2] + [intermediate_center]
        new_radii = radii[:2] + [intermediate_radius]
        new_modes = modes
        tangent_points = self._find_tangent_points_CSC(new_centers, new_modes, new_radii)
        new_centers += [None, centers[2]]
        new_radii += [np.inf, radii[2]]
        new_modes += ['S', modes[2]]
        tangent_points += self._find_tangent_points_CSC(new_centers[-3:], new_modes[-3:], new_radii[-3:])
        return tangent_points, new_centers, new_radii, new_modes
...
def utest_widget():
    ...
    for estimator in pose_estimators.values(): estimator.presence_detector.detection_threshold = 200. #4.
